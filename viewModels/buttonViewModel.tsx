...
class ButtonViewModel {
    ...
    @action public handleButtonPress = (input: ButtonTitle): void => {
        ...
        if (calculatorViewModel.input !== "0" && !input.match(/[0-9]/g)) {
            if (calculatorViewModel.input.at(-1) === "," || calculatorViewModel.input.at(-1) === "+" || calculatorViewModel.input.at(-1) === "÷" || calculatorViewModel.input.at(-1) === "−" || calculatorViewModel.input.at(-1) === "×") {
    ...
    @action public erase = (): void => {
        calculatorViewModel.calculators.find(env => (env.uuid === calculatorViewModel.selectedUUID))!.input = "0";
    }

    @action public undo = (): void => {
        const currentCalculator = calculatorViewModel.calculators.find(env => (env.uuid === calculatorViewModel.selectedUUID));
    ...
    @action public equal = (): void => {
        let tmpInput: string = calculatorViewModel.input.replaceAll("÷", "/").replaceAll("×", "*").replaceAll("−", "-").replaceAll(",", ".");
